...
uint8_t pump_operation_time[20][13];//20条，每条第一个字节为0，代表无效，1：代表有效
uint8_t pump_operation_time_write_pointer=0,pump_operation_time_read_pointer=0;//记录和保存位置指针
...
unsigned char    ACTION_TYPE=1;                 //1:动作过程中不检测鼾声，直接充到饱。     2.动作过程中检测鼾声，鼾声停则停止充气，鼾声起则继续充气。  3.动作过程中检测鼾声，鼾声停则立即放气  4.抬高头部
...
//		  			gStorageData.Standby=1;
		  ...
		  if(Airsensor.AirSensor_step_stable_flag==1 && Airsensor.AirSensor_step_last_stable_flag==1)
           ...
           xSemaphoreGive( xPositionMute );
              ...
              osDelay(1);
 			     ...
 			     Position_State_Before_Recheck=Airsensor.AirSensor_PersonOn;//保存复检前的是否有人状态，跟复检结果比较
	...
	else
	{
	  ...
	  //最近一次的判断为有人是由03位置检测判断的，由于有死区，在10分钟内如果无鼾声，需要重新采用03位置检测再次确认一下是否有人在床
    ...
    if((PillowControl.SnoreOn==false)&&(Airsensor.personon_by_positioncheck_flag==1)&&(AppTest==0))//&&(stimestructure.Hours>5))			
	  {
			t2++;
			if((t2>AirSensor_recheck_person_onoff_time*60*10))  //标志有效一定时间后重检确认        //&&((stimestructure.Hours>=8)&&(stimestructure.Hours<22)))  //10分钟内无鼾				
			{
				 t2=0;
				 //进行位置检测，再次确认是否有人
...
//					  gStorageData.Standby=0; 
	...
	//临时增加，MP2 gStorageData.Standby始终为0即工作状态
...
//		      InhaleTime=0;
//					ExhaleTime=0;
//					memset(&gStorageData.Snore,0,sizeof(SnoreData_def)); //为加晚上无人也检测鼾声临时关闭
//					gStorageData.Snore.UsedRecord=0x55aa;
//					gStorageData.Snore.StudyMode=gStorageData.StudyMode;
//					gStorageData.Standby=0;
//					ProcessControl.tick=0;
...
//		 }
		 ...
		 //修改于2020.06.08
		 if(gStorageData.StudyMode==1 || gStorageData.StudyMode==2)   //StudyMode 0:工作模式 1：学习模式 2：演示模式
		 {
				continue;
		 }
		
		 //为了晚上12：00有鼾声也检测位置临时打开，原本关闭的
		
		 #if 1 //disabled by zjp at 20190505 移到上面进行处理
				if(gStorageData.Standby)
				{
			...
			if(step==0)
			{
              ...
              osDelay(1);
				    ...
				    Airsensor.AirSensor_Checkposition_Mode=ONCE_TIME;
						...
						while(Airsensor.AirSensor_Checkposition_Mode==ONCE_TIME)
							osDelay(100);
						if(RequestControlValve!=3) //锁定止鼾动作使用气阀等
							osDelay(50);
              ...
              osDelay(1);
						...
						{	
							InhaleTime = 0;
				      ActionSteps = 0;
				      RequestControlValve=0;//解除对气阀等的占用
				      PillowControl.PillowAction = false;
				      ProcessControl.on = false;
			        ProcessControl.tick = 0;							
							StopAction();
...
/* disabled by zjp,以便始终推动最大位置点，否则如果此次最大位置仍是上次的位置，则会充气第二大的位置	
//------------------------------------------------------------------------------------------------------------						
...
*/					
						...
						switch(HeadPosition.head1)
						{
								...
								default:
								  ...
								  RequestControlValve=0;//无位置，恢复状态
								  ActionSteps=0;
								  {
										//新增20201224  有鼾声只检一次位置，然后回人体检测模式
										PillowControl.SnoreOn=false;
			        ...
			        ProcessControl.tick = 0;
				        ...
				        if(ACTION_TYPE==2)
				        {
				        ...
				        else if(ACTION_TYPE==1)
								{
									uint8_t temp_flag=0;
								  Antisnore_count_timer=0;
								  Airsensor.antisnore_timer_len=60;//检测鼾声，无鼾声延时60秒
								  Airsensor.antisonre_timer_flag=1;
...
//			    HAL_RTC_GetDate(&hrtc, &sdatestructure_snore, RTC_FORMAT_BIN);
					...
					pump_operation_time[pump_operation_time_write_pointer][0]=0x1;	//有效该次时间记录					
...
//					Airsensor_Print_RTC_Time_Actiontask1();				
					...
					uint8_t temp_flag=0;
					Antisnore_count_timer=0;
				  Airsensor.antisnore_timer_len=10;//检测鼾声，无鼾声延时60秒
					Airsensor.antisonre_timer_flag=1;
					while(Airsensor.antisonre_timer_flag==1)
					{
...
//					  continue;	
					...
					else
					{
					  //osDelay(PILLOW_AIROFFTIME-11000);
						temp_flag=0;
					  Antisnore_count_timer=0;
				    Airsensor.antisnore_timer_len=(PILLOW_AIROFFTIME-11000)/1000;//检测鼾声，无鼾声延时60秒
					  Airsensor.antisonre_timer_flag=1;
					  while(Airsensor.antisonre_timer_flag==1)
					  {
