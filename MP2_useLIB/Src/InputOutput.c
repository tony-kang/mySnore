...
void ValueDriver(unsigned char value,GPIO_PinState s)
{
	...
	ValueDriver1(value,s);//不采取降低气阀噪音方案
	
#if 0 //降低气阀噪音操作
  //****************** added by zjp ****************
	if(s==GPIO_PIN_SET)
	{
		if(Valve_State[value-1]==0)
		{
		  Switch_Valve_Do_Airbag_To_Pump(value);
			Valve_State[value-1]=1;
		}
		
	}
	else
	{
		if(Valve_State[value-1]==1)
		{
		  Switch_Valve_Do_Airbag_To_Air(value);
			Valve_State[value-1]=0;
		}
	}
#endif 
#if 0	
